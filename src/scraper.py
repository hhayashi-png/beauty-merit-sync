...
def _set_current_month(page: Page) -> None:
    ...
    try:
        ...
        if year_sel and month_sel:
            year_sel.select_option(str(y))
            month_sel.select_option(str(m))
            logger.info(f"当月設定(年月select): {y}-{m:02d}")
            return
    except Exception as e:
    ...
    try:
        ...
        if ym_sel:
            ym_sel.select_option(f"{y}-{m:02d}")
            logger.info(f"当月設定(ym select): {y}-{m:02d}")
            return
    except Exception as e:
    ...
    try:
        ...
        if inp:
            inp.fill(f"{y}-{m:02d}")
            logger.info(f"当月設定(input): {y}-{m:02d}")
            return
    except Exception as e:
        logger.warning(f"month input失敗: {e}")

    logger.warning(
        "⚠️ 年月選択UIが見つかりませんでした。デフォルト(おそらく当月)のまま続行します。"
    )
    _dump(page, "no_month_selector")
...
def _run_once(
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            _set_current_month(page)
            return _download_zip(page, save_dir)
        except Exception:
            # 想定外失敗時は必ずデバッグダンプ
            try:
                _dump(page, "unexpected_failure")
            except Exception:
                pass
            raise
        finally:
...
def download_reservations_zip(
    ...
    """当月の予約ZIPを取得(指数バックオフでリトライ)"""
    save_dir = Path(save_dir)
    last_err: Exception | None = None

    for attempt in range(1, MAX_RETRIES + 1):
        try:
            logger.info(f"ダウンロード試行 {attempt}/{MAX_RETRIES}")
            return _run_once(login_id, password, save_dir, headless, debug_dump)
        except Exception as e:
            last_err = e
    ...
    raise RuntimeError(f"ZIPダウンロードに{MAX_RETRIES}回失敗: {last_err}") from last_err
